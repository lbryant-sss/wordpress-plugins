...
class Device_Browsers extends \IAWP\Rows\Rows
{
    ...
    private function query(?bool $skip_pagination = \false) : Builder
    {
        ...
        })->whereNotNull('sessions.device_browser_id')->groupBy('sessions.session_id', 'sessions.old_visitor_id', 'sessions.visitor_id', 'sessions.initial_view_id', 'sessions.referrer_id', 'sessions.campaign_id', 'sessions.created_at', 'sessions.legacy_view', 'sessions.final_view_id', 'sessions.ended_at', 'sessions.city_id', 'sessions.country_id', 'sessions.total_views', 'sessions.device_type_id', 'sessions.device_os_id', 'sessions.device_browser_id', 'sessions.is_first_session');
        ...
        })->groupBy('device_browsers.device_browser_id', 'device_browsers.device_browser')->having('views', '>', 0)->when(!$this->is_using_a_calculated_column(), function (Builder $query) {
