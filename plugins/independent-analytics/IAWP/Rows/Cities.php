...
class Cities extends \IAWP\Rows\Rows
{
    ...
    private function query(?bool $skip_pagination = \false) : Builder
    {
        ...
        })->whereNotNull('sessions.city_id')->groupBy('sessions.session_id', 'sessions.old_visitor_id', 'sessions.visitor_id', 'sessions.initial_view_id', 'sessions.referrer_id', 'sessions.campaign_id', 'sessions.created_at', 'sessions.legacy_view', 'sessions.final_view_id', 'sessions.ended_at', 'sessions.city_id', 'sessions.country_id', 'sessions.total_views', 'sessions.device_type_id', 'sessions.device_os_id', 'sessions.device_browser_id', 'sessions.is_first_session');
        ...
        })->groupBy('cities.city_id', 'cities.subdivision', 'cities.city', 'cities.city_id', 'countries.country_id', 'countries.country_code', 'countries.country', 'countries.continent')->having('views', '>', 0)->when(!$this->is_using_a_calculated_column(), function (Builder $query) {
