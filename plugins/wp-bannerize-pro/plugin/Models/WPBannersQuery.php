...
<?php
  ...
  public function posts_where_request($where)
  {
    ...
    $data_impressions = 'wp_bannerize_pm_wp_bannerize_banner_impressions';
    $data_max_impressions = 'wp_bannerize_pm_wp_bannerize_banner_max_impressions';
    $data_clicks = 'wp_bannerize_pm_wp_bannerize_banner_clicks';
    $data_max_clicks = 'wp_bannerize_pm_wp_bannerize_banner_max_clicks';
    ...
    // max impressions
    $where .= " AND ( COALESCE({$data_max_impressions}.meta_value, '0') = '0' OR {$data_impressions}.meta_value + 0 < {$data_max_impressions}.meta_value + 0 ) ";

    // max clicks
    $where .= " AND ( COALESCE({$data_max_clicks}.meta_value, '0') = '0' OR {$data_clicks}.meta_value + 0 < {$data_max_clicks}.meta_value + 0 ) ";
  ...
  public function posts_join_request($join)
  {
    ...
    // max Impressions
    $meta_key = 'wp_bannerize_banner_max_impressions';
    $pm = 'wp_bannerize_pm_' . $meta_key;
    // Remember to keep a space at begin and end of string
    $joins[] = " LEFT JOIN {$wpdb->postmeta} AS {$pm} ON ( {$pm}.post_id = {$wpdb->posts}.ID AND {$pm}.meta_key = '{$meta_key}' ) ";

    // max Clicks
    $meta_key = 'wp_bannerize_banner_max_clicks';
    $pm = 'wp_bannerize_pm_' . $meta_key;
    // Remember to keep a space at begin and end of string
    $joins[] = " LEFT JOIN {$wpdb->postmeta} AS {$pm} ON ( {$pm}.post_id = {$wpdb->posts}.ID AND {$pm}.meta_key = '{$meta_key}' ) ";
