...
(function (AmeContentPermissionsUi) {
    ...
    class ContentPermissionsEditor {
        constructor(editorData) {
            ...
            this.policy = new Policy(editorData.policy || {}, this.advancedTabActors(), this.actions, editorData.requiredCapabilities);
            ...
            this.basicViewState = ko.computed({
                read: () => {
                    ...
                    if (this.policy.matchesMultiplePermissions(genericLoggedInUser, presets.loggedIn.on)
                    ...
                    if (this.policy.matchesMultiplePermissions(genericLoggedInUser, presets.loggedIn.off)
        ...
        getActorPermission(action, actor = null) {
        ...
        getPredictedDefaultPermission(action, actor = null) {
            const targetActor = actor || this.selectedActor();
            if (!targetActor) {
                return null;
            }
    ...
    class Policy {
        constructor(properties, validActors, validActions, requiredCapsByAction = {}) {
        ...
        setMultiplePermissions(actor, permissions) {
            for (const [action, effect] of permissions) {
                this.setActorPermission(actor, action, effect);
            }
        }
        matchesMultiplePermissions(actor, permissions) {
            for (const [action, effect] of permissions) {
    ...
    class MiniGridItem {
        constructor(actor, action, editor) {
            this.cssClass = ko.computed(() => {
                const classes = [];
                if (!action.isVisibleFor(actor)) {
