...
i18n.COMMON = {
  ...
  USAGE_HELP: __( 'The values displayed under the Usage section in the Dashboard are informational estimates only and shouldn\'t be used for billing. Token counts and costs vary significantly across services; please check your service accounts for exact amounts. The "Insights" module has a better accuracy.', 'ai-engine' ),
...
i18n.SETTINGS = {
  ...
  REMOTE_ACCESS_INFO: __( 'AI Engine provides two ways to integrate with external systems: <b>REST API</b> and <b>Model Context Protocol (MCP)</b>.<br/><br/><b>REST API:</b> Ideal for automation platforms like Make.com, Zapier, or n8n. Use this when you want to trigger AI operations from external services or integrate AI Engine into your existing workflows.<br/><br/><b>MCP:</b> Transforms your WordPress into an MCP server that Claude (the app) can connect to directly. Provides access to Tuned Core (WordPress management), Tuned Plugins (plugin development), and integrates with other tools like <a href="https://meowapps.com/seo-engine/" target="_blank">SEO Engine</a> and <a href="https://wordpress.org/plugins/code-engine/" target="_blank">Code Engine</a> for snippet handling. Developers can extend functionality using the <code>mwai_mcp_tools</code> and <code>mwai_mcp_callback</code> filters (see dev-notes.md).<br/><br/><b>Note:</b> This MCP feature makes WordPress an MCP server for external AI agents. It\'s different from the Orchestration module, which allows AI Engine to connect to external MCP servers.<br/><br/>Choose REST API for workflow automation. Choose MCP to give AI agents direct access to your WordPress.', 'ai-engine' ),
  KNOWLEDGE_INFO: __( 'An embedding is a textual piece of data (sentence, paragraph, a whole article) that has been converted by OpenAI into a vector. Vectors can then be used to find the most relevant data for a given query. In this dashboard, you can create embeddings, and they will be synchronized with <a target="_blank" href="https://www.pinecone.io/">Pinecone</a>, a very fast and affordable vector database.<br/><br/>You can switch from EDIT to AI SEARCH and you will be able to query the database, and get your content, with a score. You can edit the content and it will be synchronized with Pinecone. Then make your content perfect so that the results are satisfying! You can use Sync and Sync One, it will go through your posts and create the embeddings if they don\'t exist yet, or update them if they do.<br/><br/>Both the chatbots and the AI Forms can use embeddings to enrich their answers.<br/><br/><b>If you want to discuss embeddings with other users, visit the <a href="https://discord.gg/bHDGh38" target="_blank">Meow Apps Discord Server</a>.</b>', 'ai-engine' ),
