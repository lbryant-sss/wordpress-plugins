...
class MCProtectIpstore_V592 {
	...
	function __construct($storage_type = MCProtectIpstore_V592::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V592::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V592();
		} else {
			$this->storage = new MCProtectIpstoreDB_V592();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V592::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V592::STORAGE_TYPE_DB) {
