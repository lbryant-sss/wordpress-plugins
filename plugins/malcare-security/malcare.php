...
if ((array_key_exists('bvplugname', $_REQUEST)) && ($_REQUEST['bvplugname'] == "malcare")) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	if ($bvinfo->hasValidDBVersion()) {
		if ($bvinfo->isProtectModuleEnabled()) {
			...
			MCProtect_V588::$settings = new MCWPSettings();
			MCProtect_V588::$db = new MCWPDb();
			MCProtect_V588::$info = new MCInfo(MCProtect_V588::$settings);

			add_action('mc_clear_pt_config', array('MCProtect_V588', 'uninstall'));

			if ($bvinfo->isActivePlugin()) {
				MCProtect_V588::init(MCProtect_V588::MODE_WP);
			}
		}
