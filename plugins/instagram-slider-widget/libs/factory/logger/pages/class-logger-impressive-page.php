 ...
 * @version       1.0
 */
class Wbcr_FactoryLogger150_PageBase extends \WBCR\Factory_Templates_135\Pages\PageBase {
	...
	public function ajax_cleanup() {
		...
		if ( ! $this->plugin->logger->clean_up() ) {
			wp_send_json_error( [
				'message' => __( 'Failed to clean-up logs. Please try again later.', 'wbcr_factory_logger_150' ),
				'type'    => 'danger',
			] );
		}

		wp_send_json( [
			'message' => __( 'Logs clean-up successfully', 'wbcr_factory_logger_150' ),
			'type'    => 'success',
		] );
	}
