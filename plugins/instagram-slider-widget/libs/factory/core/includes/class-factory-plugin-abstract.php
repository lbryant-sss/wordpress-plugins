 ...
 *
 */
abstract class Wbcr_Factory478_Plugin extends Wbcr_Factory478_Base {

	/**
	 * Instance class Wbcr_Factory478_Request, required manages http requests
	 *
	 * @see https://webcraftic.atlassian.net/wiki/spaces/FFD/pages/390561806
	 * @var Wbcr_Factory478_Request
	 */
	public $request;

	/**
	 * @see https://webcraftic.atlassian.net/wiki/spaces/FFD/pages/393936924
	 * @var \WBCR\Factory_478\Premium\Provider
	 */
	public $premium;
	 ...
	 *
	 * @var Wbcr_FactoryBootstrap480_Manager
	 */
	public $bootstrap;
	 ...
	 * The Bootstrap Manager class
	 *
	 * @var Wbcr_FactoryForms478_Manager
	 */
	public $forms;

	/**
	 * Простой массив со списком зарегистрированных классов унаследованных от Wbcr_Factory478_Activator.
	 * Классы активации используются для упаковки набора функций, которые нужно выполнить во время
	 * активации плагина.
	 *
	 * @var array[] Wbcr_Factory478_Activator
	 */
	 ...
	 * @author Alexander Kovalev <alex.kovalevv@gmail.com>
	 * @since  4.1.9
	 * @var WBCR\Factory_Adverts_156\Base
	 */
	private $adverts;
	 ...
	 * @since  4.3.7
	 * @var WBCR\Factory_Logger_145\Logger
	 */
	public $logger;
	...
	public function __construct($plugin_path, $data)
	{
		...
		$this->request = new Wbcr_Factory478_Request();
		//$this->route = new Wbcr_Factory478_Route();
		...
		if( wp_doing_ajax() && isset($_REQUEST['action']) ) {
			if( "wfactory-478-intall-component" == $_REQUEST['action'] ) {
				add_action('wp_ajax_wfactory-478-intall-component', [$this, 'ajax_handler_install_components']);
			}

			if( "wfactory-478-prepare-component" == $_REQUEST['action'] ) {
				add_action('wp_ajax_wfactory-478-prepare-component', [$this, 'ajax_handler_prepare_component']);
			}
			if( "wfactory-478-creativemotion-install-plugin" == $_REQUEST['action'] ) {
				add_action('wp_ajax_wfactory-478-creativemotion-install-plugin', [
					$this,
	...
	{
		require_once FACTORY_478_DIR . '/ajax/install-addons.php';
		wfactory_478_install_components($this);
	}

	public function ajax_handler_prepare_component()
	{
		require_once FACTORY_478_DIR . '/ajax/install-addons.php';
		wfactory_478_prepare_component($this);
	}

	public function ajax_handler_install_creativemotion_plugins()
	{
		require_once FACTORY_478_DIR . '/ajax/install-addons.php';
		wfactory_478_creativemotion_install_plugin($this);
	}
	// --------------------------------------------------------
	 ...
	 *
	 * @param Wbcr_FactoryBootstrap480_Manager $bootstrap
	 */
	public function setBootstap(Wbcr_FactoryBootstrap480_Manager $bootstrap)
	{
	 ...
	 *
	 * @param Wbcr_FactoryForms478_Manager $forms
	 */
	public function setForms(Wbcr_FactoryForms478_Manager $forms)
	{
		$this->forms = $forms;
	}
	...
	{
		if( !isset(WBCR\Factory_478\Premium\Manager::$providers[$name]) ) {
			WBCR\Factory_478\Premium\Manager::$providers[$name] = $class_name;
		}
	}
	...
	{
		if( !isset(WBCR\Factory_478\Updates\Upgrader::$repositories[$name]) ) {
			WBCR\Factory_478\Updates\Upgrader::$repositories[$name] = $class_name;
		}
	}
	 ...
	 *
	 * @return \WBCR\Factory_Adverts_156\Base
	 * @since  1.1
	 * @author Alexander Kovalev <alex.kovalevv@gmail.com>
	 */
	...
	{
		return new Wbcr_Factory478_ScriptList($this);
	}

	public function newStyleList()
	{
		return new Wbcr_Factory478_StyleList($this);
	}
	 ...
	 * Wbcr_FactoryPages478_AdminPage, а затем регистрирует его через этот метод.
	 ...
	 *                             Регистрируемый класс должен быть унаследован от класса Wbcr_FactoryPages478_AdminPage.
	...
	public function registerPage($class_name, $file_path)
	{
		...
		if( !class_exists('Wbcr_FactoryPages478') ) {
			throw new Exception('The factory_pages_478 module is not included.');
		}

		Wbcr_FactoryPages478::register($this, $class_name);
	}
	...
	public function activation_hook()
	{
		 ...
		 */
		if( apply_filters("wbcr/factory_478/cancel_plugin_activation_{$this->plugin_name}", false) ) {
			return;
		}

		/**
		 * wbcr_factory_478_plugin_activation
		 *
		 * @since 4.1.1 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr_factory_478_plugin_activation', [
			$this
		 ...
		 */
		wbcr_factory_478_do_action_deprecated('wbcr/factory/plugin_activation', [
			$this
		 ...
		 * @since 4.1.2 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated("wbcr/factory/plugin_{$this->plugin_name}_activation", [
			$this
		], '4.1.2', "wbcr/factory/before_plugin_{$this->plugin_name}_activation");

		/**
		 * wbcr_factory_478_plugin_activation_' . $this->plugin_name
		 *
		 * @since 4.1.1 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr_factory_478_plugin_activation_' . $this->plugin_name, [
			$this
	...
	public function deactivation_hook()
	{
		 ...
		 * @since 4.1.1 - change  hook name
		 */
		if( apply_filters("wbcr/factory_478/cancel_plugin_deactivation_{$this->plugin_name}", false) ) {
			return;
		}

		/**
		 * wbcr_factory_478_plugin_deactivation
		 *
		 * @since 4.1.1 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr_factory_478_plugin_deactivation', [
			$this
		 ...
		 * @since 4.1.2 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr/factory/plugin_deactivation', [
			$this
		...
		/**
		 * wbcr_factory_478_plugin_deactivation_ . $this->plugin_name
		 *
		 * @since 4.1.1 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr_factory_478_plugin_deactivation_' . $this->plugin_name, [
			$this
		 ...
		 * @since 4.1.2 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated("wbcr/factory/plugin_{$this->plugin_name}_deactivation", [
			$this
	...
	public function getPluginPageUrl($page_id, $args = [])
	{
		...
		return Wbcr_FactoryPages478::getPageUrl($this, $page_id, $args);
	}
	...
	{
		require_once FACTORY_478_DIR . '/includes/components/class-install-component-button.php';

		return new \WBCR\Factory_478\Components\Install_Button($this, $component_type, $slug);
	}
	...
	{
		require_once FACTORY_478_DIR . '/includes/components/class-delete-component-button.php';

		return new WBCR\Factory_478\Components\Delete_Button($this, $component_type, $slug);
	}
	...
	private function init_framework_modules()
	{
		 ...
		 * @since 4.1.1 - deprecated
		 */
		wbcr_factory_478_do_action_deprecated('wbcr_factory_478_core_modules_loaded-' . $this->plugin_name, [], '4.1.1', "wbcr/factory_478/modules_loaded-" . $this->plugin_name);

		/**
		 * @since 4.1.1 - add
		 */
		do_action('wbcr/factory_478/modules_loaded-' . $this->plugin_name);
	}
	...
	private function register_plugin_hooks()
	{
		...
		if( is_admin() ) {
			add_filter('wbcr_factory_478_core_admin_allow_multisite', '__return_true');
	...
	{
		new WBCR\Factory_478\Migrations($this);
	}
	...
	{
		new Wbcr\Factory_478\Notices($this);
	}
	...
	{
		if( $this->has_updates ) {
			new WBCR\Factory_478\Updates\Upgrader($this);
		}
	}
	...
	protected function init_plugin_premium_features()
	{
		...
		$this->premium = WBCR\Factory_478\Premium\Manager::instance($this, $this->license_settings);

		// Подключаем премиум апгрейдер
		if( isset($this->license_settings['has_updates']) && $this->license_settings['has_updates'] ) {
			new WBCR\Factory_478\Updates\Premium_Upgrader($this);
		}
	}
}
