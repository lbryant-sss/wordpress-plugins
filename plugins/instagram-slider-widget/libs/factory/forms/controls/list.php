<?php
	...
	if( !class_exists('Wbcr_FactoryForms481_ListControl') ) {

		class Wbcr_FactoryForms481_ListControl extends Wbcr_FactoryForms481_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo esc_attr($ajax_id); ?>'] = {
							...
							'emptyList': '<?php echo esc_html($this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_481') )); ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_481'));
