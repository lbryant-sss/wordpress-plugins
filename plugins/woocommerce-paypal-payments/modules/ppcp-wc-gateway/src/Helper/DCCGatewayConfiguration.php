...
class DCCGatewayConfiguration
{
    ...
    {
        $is_paypal_enabled = $this->settings->has('enabled') && filter_var($this->settings->get('enabled'), \FILTER_VALIDATE_BOOLEAN);
        $is_dcc_enabled = $this->settings->has('dcc_enabled') && filter_var($this->settings->get('dcc_enabled'), \FILTER_VALIDATE_BOOLEAN);
        $is_axo_enabled = $this->settings->has('axo_enabled') && filter_var($this->settings->get('axo_enabled'), \FILTER_VALIDATE_BOOLEAN);
        $this->is_enabled = $is_paypal_enabled && $is_dcc_enabled;
        $this->use_fastlane = $this->is_enabled && $is_axo_enabled;
        $this->gateway_title = $this->settings->has('dcc_gateway_title') ? $this->settings->get('dcc_gateway_title') : '';
        $this->gateway_description = $this->settings->has('dcc_gateway_description') ? $this->settings->get('dcc_gateway_description') : '';
        $show_on_card = '';
        ...
        $this->show_name_on_card = in_array($show_on_card, $valid_options, \true) ? $show_on_card : $valid_options[0];
        /**
