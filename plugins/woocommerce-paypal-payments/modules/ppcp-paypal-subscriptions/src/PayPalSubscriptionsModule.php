...
class PayPalSubscriptionsModule implements ServiceModule, ExtendingModule, ExecutableModule
{
    ...
    public function run(ContainerInterface $c): bool
    {
        add_action(
            ...
            function ($product_id) use ($c) {
                ...
                if ($subscriptions_mode !== 'subscriptions_api' || !is_string($nonce) || !wp_verify_nonce($nonce, 'wcs_subscription_meta')) {
                    return;
                }
        ...
        add_action(
            ...
            function ($variation_id) use ($c) {
                ...
                $subscriptions_helper = $c->get('wc-subscriptions.helper');
                assert($subscriptions_helper instanceof SubscriptionHelper);
                if (!$subscriptions_helper->plugin_is_active() || !WC_Subscriptions_Product::is_subscription($variation_id) || !is_string($wcsnonce_save_variations) || !wp_verify_nonce($wcsnonce_save_variations, 'wcs_subscription_variations')) {
                    return;
                }
        ...
        add_filter(
            ...
            function ($actions, $subscription): array {
                ...
                if ($subscription_id && $subscription->get_status() === 'active') {
                    $url = wp_nonce_url(add_query_arg(array('change_subscription_to' => 'cancelled', 'ppcp_cancel_subscription' => $subscription->get_id())), 'ppcp_cancel_subscription_nonce');
                    array_unshift($actions, array('url' => esc_url($url), 'name' => esc_html__('Cancel', 'woocommerce-paypal-payments')));
        ...
        add_action('wp_loaded', function () use ($c) {
            ...
            if ($subscription_id && $cancel_subscription_id && $nonce) {
                if (!wp_verify_nonce($nonce, 'ppcp_cancel_subscription_nonce') || !user_can(get_current_user_id(), 'edit_shop_subscription_status', $subscription->get_id())) {
                    return;
                }
        ...
        add_action(
            ...
            function ($hook) use ($c) {
                ...
                $subscriptions_helper = $c->get('wc-subscriptions.helper');
                assert($subscriptions_helper instanceof SubscriptionHelper);
                if (!$subscriptions_helper->plugin_is_active() || !(is_a($product, WC_Product_Subscription::class) || is_a($product, WC_Product_Variable_Subscription::class) || is_a($product, WC_Product_Subscription_Variation::class)) || !WC_Subscriptions_Product::is_subscription($product)) {
                    return;
                }
                $module_url = $c->get('paypal-subscriptions.module.url');
                wp_enqueue_script('ppcp-paypal-subscription', untrailingslashit($module_url) . '/assets/js/paypal-subscription.js', array('jquery', 'wc-admin-product-editor'), $c->get('ppcp.asset-version'), \true);
                ...
                wp_localize_script('ppcp-paypal-subscription', 'PayPalCommerceGatewayPayPalSubscriptionProducts', $products);
            }
        );
    ...
    private function set_product_config(WC_Product $product): array
    {
        ...
        $plan_id = $plan['id'] ?? '';
        return array('product_connected' => $product->get_meta('_ppcp_enable_subscription_product') ?? '', 'plan_id' => $plan_id, 'product_id' => $product->get_id(), 'ajax' => array('deactivate_plan' => array('endpoint' => \WC_AJAX::get_endpoint(\WooCommerce\PayPalCommerce\PayPalSubscriptions\DeactivatePlanEndpoint::ENDPOINT), 'nonce' => wp_create_nonce(\WooCommerce\PayPalCommerce\PayPalSubscriptions\DeactivatePlanEndpoint::ENDPOINT))));
    }
    /**
