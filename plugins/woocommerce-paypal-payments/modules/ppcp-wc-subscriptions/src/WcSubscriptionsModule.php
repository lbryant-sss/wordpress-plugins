    ...
    public function run(ContainerInterface $c): bool
    {
        ...
        add_filter(
            ...
            function ($is_supported, $feature, $subscription) {
                if ($subscription->get_payment_method() === PayPalGateway::ID && $feature === 'gateway_scheduled_payments') {
    ...
    private function add_gateways_support(ContainerInterface $c): void
    {
        add_filter('woocommerce_paypal_payments_paypal_gateway_supports', function (array $supports) use ($c): array {
            ...
            if ('disable_paypal_subscriptions' !== $subscriptions_mode && $subscriptions_helper->plugin_is_active()) {
                $supports = array('subscriptions', 'subscription_cancellation', 'subscription_suspension', 'subscription_reactivation', 'subscription_amount_changes', 'subscription_date_changes', 'subscription_payment_method_change', 'subscription_payment_method_change_customer', 'subscription_payment_method_change_admin', 'multiple_subscriptions', 'gateway_scheduled_payments');
            }
            return $supports;
        });
