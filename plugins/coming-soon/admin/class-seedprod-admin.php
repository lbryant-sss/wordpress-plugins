...
class SeedProd_Lite_Admin {
	...
	public function handle_subscribers_tab_redirect() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'seedprod_lite_settings' &&
		     isset( $_GET['tab'] ) && $_GET['tab'] === 'subscribers' ) {
	...
	public function handle_old_dashboard_redirect() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'seedprod_lite_dashboard' ) {
	...
	public function display_landing_pages_page() {
		...
		wp_localize_script( 'seedprod-admin', 'seedprod_admin', array(
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'admin_url' => admin_url(),
			'nonce' => wp_create_nonce( 'seedprod_nonce' ),
			'v2_nonce' => wp_create_nonce( 'seedprod_v2_nonce' ),
	...
	/**
	 * Render the Smart Builder page
	 */
	public function display_smart_builder_page() {
		$admin_page_path = plugin_dir_path( dirname( __FILE__ ) ) . 'sp-smart-builder/admin-page.php';

		if ( file_exists( $admin_page_path ) ) {
			require_once $admin_page_path;
			seedprod_lite_render_smart_builder_page();
		} else {
			// Smart Builder not available (excluded from build)
			echo '<div class="wrap"><h1>Smart Builder</h1><p>Smart Builder is not available in this build.</p></div>';
		}
	}
	...
	public function fix_menu_highlighting_js() {
		...
		if ( ! isset( $plugin_page ) || $plugin_page !== 'seedprod_lite_subscribers' ) {
			return;
		}
