...
class FluentBlockEditorHandler
{
    ...
    public function register()
    {
        add_action('init', function () {
            ...
            if (isset($_REQUEST['fluent_crm_block_editor'])) {
                ...
                $required_cap = 'fcrm_read_emails';
                if ($block_type === 'campaign') {
                    $required_cap = 'fcrm_manage_emails';
                } elseif ($block_type === 'recurring_campaign') {
                    $required_cap = 'fcrm_manage_emails';
                } elseif ($block_type === 'template') {
                    $required_cap = 'fcrm_manage_email_templates';
                }
                /**
                ...
                add_action('fluent_crm/block_editor_head', function () {
                    $url = FLUENTCRM_PLUGIN_URL . 'Modules/Gutenberg/editor/index.css';
                    ?>
                    <link rel="stylesheet" href="<?php echo esc_url($url); ?>?version=<?php echo esc_attr(FLUENTCRM_PLUGIN_VERSION); ?>" media="screen"/>
                    <?php
                });
    ...
    public function renderLessonEditor($data = [])
    {
        ...
        if ($context === 'campaign') {
            $required_cap = 'fcrm_manage_emails';
        } elseif ($context === 'recurring_campaign') {
            $required_cap = 'fcrm_manage_emails';
        ...
        if ($context == 'recurring_campaign') {
            $campaignId = (int) Arr::get($data, 'bid');
            if ($campaignId && $campaignId != 'undefined') {
                $campaign = RecurringCampaign::find($campaignId);
                if ($campaign) {
                    $post_content = $campaign->email_body;
                    $post_title   = $campaign->title;
                    $recordId = $campaign->id;
        ...
        if ($firstPost) {
            ...
            $simulatedPost->post_content = $post_content;
            $simulatedPost->post_title = $post_title;
        } else {
        ...
        // Prepare editor boot data for JS
    ...
    public function gutenberg_editor_scripts_and_styles($hook, $post)
    {
        ...
        */
        ...
        wp_register_style('fluent_com_editor_styles', FLUENTCRM_PLUGIN_URL . 'Modules/Gutenberg/editor/style.css', false, FLUENTCRM_PLUGIN_VERSION, 'all');

        //   add_editor_style('fluent_com_editor_styles.css');

        // wp_enqueue_style('fluent_com_editor_styles', FLUENTCRM_PLUGIN_URL . 'Modules/Gutenberg/editor/style-editor.css', false, FLUENTCRM_PLUGIN_VERSION, 'all');
...
//        add_action('fluent_enqueue_block_editor_assets', 'wp_enqueue_global_styles_css_custom_properties');
        ...
        wp_enqueue_script('fcrm_editor_custom', FLUENTCRM_PLUGIN_URL . 'Modules/Gutenberg/editor/index.js', ['react', 'wp-components', 'wp-compose', 'wp-data', 'wp-edit-post', 'wp-i18n', 'wp-plugins'], FLUENTCRM_PLUGIN_VERSION . time(), true);
        
        // Inject smartcodes into the iframe window
        $globalSmartCodes = \FluentCrm\App\Services\Helper::getGlobalSmartCodes();
        wp_add_inline_script(
            'fcrm_editor_custom',
            'window.fcAdmin = window.fcAdmin || {}; window.fcAdmin.globalSmartCodes = ' . wp_json_encode($globalSmartCodes) . ';',
            'before'
        );
    ...
    public function handleEditorAutosave(\WP_REST_Request $request)
    {
        ...
        if (empty($blockType) && !$entityId) {
            return new \WP_REST_Response([
                'status'  => 'skipped',
                'message' => 'No block_type provided; nothing created.'
            ], 200);
        }

        // Capability check
        $required_cap = 'fcrm_read_emails';
        if ($blockType === 'campaign' || $blockType === 'recurring_campaign') {
            $required_cap = 'fcrm_manage_emails';
        ...
        $model = null; $contentField = null; $titleField = null;
        if ($blockType === 'campaign') {
            $model = Campaign::class; $contentField = 'email_body'; $titleField = 'title';
        } elseif ($blockType === 'recurring_campaign') {
            $model = RecurringCampaign::class; $contentField = 'email_body'; $titleField = 'title';
        } elseif ($blockType === 'template') {
            $model = Template::class; $contentField = 'post_content'; $titleField = 'post_title';
        } else {
        ...
        if ($entityId) {
            ...
            // we won't create new if id is 0 and block type empty
            // Create new record only if block type present (already ensured) and title/content provided
            /** @var \FluentCrm\Framework\Database\Model $model */
            // try {
            //     $record = $model::create([
            //         $titleField   => $title ?: __('(Untitled)', 'fluent-crm'),
        ...
        if ($blockType === 'template') {
            ...
            if ($record->$contentField !== $content) {
                $record->$contentField = $content; $dirty = true;
            }
            ...
            if ($title && $record->$titleField !== $title) {
                $record->$titleField = $title; $dirty = true;
            }
            if ($dirty) {
                try {
                    $record->save();
    ...
    private function getEditorSettings($post)
    {
        ...
        // }
    // $colorSchmeaCss .= '}';
    // Ensure variable exists even if schema generation is skipped
    $colorSchmeaCss = isset($colorSchmeaCss) ? $colorSchmeaCss : '';
        ...
        $editor_settings['styles'] = [
            ...
            [
                'css'            => file_get_contents(FLUENTCRM_PLUGIN_PATH . 'Modules/Gutenberg/editor/editor.css'),
                '__unstableType' => 'user'
            ]
        ];
