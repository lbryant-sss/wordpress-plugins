...
export const useDate = create( ( set ) => {
  ...
  return {
    startDate: rangeData ? rangeData.startDate : format(
        startOfDay( subDays( new Date(), 7 ) ), 'yyyy-MM-dd' ),
    endDate: rangeData ? rangeData.endDate : format(
        endOfDay( subDays( new Date(), 1 ) ), 'yyyy-MM-dd' ),
