    ...
    let task = props.task;
        ...
        return (
            <div className="flex items-center justify-center gap-m pb-s">
                ...
                {task.highlight_field_id && <span className="cursor-pointer" onClick={handleClick}>{__( 'Fix', 'burst-statistics' )}</span> }
                {task.plusone && <span className='inline-block align-top box-border m-[1px_0_-1px_2px] py-0 px-xxs min-w-[18px] h-[18px] rounded-[9px] bg-red text-white text-xs leading-[1.6] text-center'>1</span>}
...
const TaskStatusIcon = ( props ) => {
    return (
        <span className={clsx(
            'block min-w-[96px] text-center rounded-[15px] px-2 py-1 font-semibold text-xxs', // default color, overwrite below based on icon
