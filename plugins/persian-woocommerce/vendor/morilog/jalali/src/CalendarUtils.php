 ...
 */
class CalendarUtils {
	public const IRANIAN_MONTHS_NAME = [
		'فروردین',
		'اردیبهشت',
		'خرداد',
		'تیر',
	...
	public const AFGHAN_MONTHS_NAME = [
		'حمل',
		'ثور',
		'جوزا',
		'سرطان',
	...
	public static function j2d( $jy, $jm, $jd ) {
		...
		return self::g2d( $jCal['gy'], 3, $jCal['march'] ) + ( $jm - 1 ) * 31 - self::div( $jm,
				7 ) * ( $jm - 7 ) + $jd - 1;
	}
	 ...
	 * @param null $timezone
	 *
	 * @return DateTime|static|DateTimeInterface
	 */
	...
	public static function parseFromFormat( $format, $date ) {
		...
		foreach ( $chars as $n => $char ) {
			...
			if ( ! $skipCurrent && isset( $keys[ $char ] ) ) {
				// Handle special format characters (e.g., year, month, day, etc.)
				$regex .= '(?P<' . $keys[ $char ][0] . '>' . $keys[ $char ][1] . ')';
			} elseif ( '\\' == $char ) {
				// Skip the backslash used to escape the next character
				continue;
			} else {
				// Escape any other literal character
		...
		if ( preg_match( '#^' . $regex . '$#', $date, $dt ) ) {
			...
			if ( ! CalendarUtils::checkdate( $dt['month'], $dt['day'], $dt['year'] ) ) {
				$dt['error_count'] = 1;
			}
		} else {
			$dt['error_count'] = 1;
	...
	public static function checkDate( $year, $month, $day, $isJalali = true ) {
		return $isJalali === true ? self::isValidateJalaliDate( $year, $month, $day ) : checkdate( $month, $day,
			$year );
	}
