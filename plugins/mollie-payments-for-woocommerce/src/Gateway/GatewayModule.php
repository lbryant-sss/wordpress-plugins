...
class GatewayModule implements ServiceModule, ExecutableModule
{
    ...
    {
        return ['gateway.classnames' => static function (): array {
            ...
            return in_array(Constants::DIRECTDEBIT, $enabledMethods, \true);
        ...
        }, \Mollie\WooCommerce\Gateway\Surcharge::class => static function (ContainerInterface $container): \Mollie\WooCommerce\Gateway\Surcharge {
            return new \Mollie\WooCommerce\Gateway\Surcharge();
        ...
        }, \Mollie\WooCommerce\Gateway\OrderMandatoryGatewayDisabler::class => static function (ContainerInterface $container): \Mollie\WooCommerce\Gateway\OrderMandatoryGatewayDisabler {
            $settings = $container->get('settings.settings_helper');
            assert($settings instanceof Settings);
            $isSettingsOrderApi = $settings->isOrderApiSetting();
            return new \Mollie\WooCommerce\Gateway\OrderMandatoryGatewayDisabler($isSettingsOrderApi);
    ...
    public function run(ContainerInterface $container): bool
    {
        ...
        add_filter('woocommerce_payment_gateways', static function ($gateways) use ($container) {
            $orderMandatoryGatewayDisabler = $container->get(\Mollie\WooCommerce\Gateway\OrderMandatoryGatewayDisabler::class);
            assert($orderMandatoryGatewayDisabler instanceof \Mollie\WooCommerce\Gateway\OrderMandatoryGatewayDisabler);
        ...
        add_filter('woocommerce_payment_gateways', [$this, 'maybeDisableBankTransferGateway'], 20);
        ...
        if ($isIn3Enabled) {
            ...
            add_action('woocommerce_before_pay_action', [$this, 'in3FieldsMandatoryPayForOrder'], 11);
    ...
    public function setOrderPaidByOtherGateway($order_id)
    {
        ...
        $mollie_payment_id = $order->get_meta('_mollie_payment_id', $single = \true);
     ...
     */
    public function molliePayPalButtonHandling($gateway, NoticeInterface $notice, Logger $logger, string $pluginUrl)
    {
    ...
    public function instantiatePaymentMethodGateways(ContainerInterface $container): array
    {
        ...
        foreach ($paymentMethods as $paymentMethod) {
            ...
            if ($isSepa) {
                ...
                $gateways[$key] = new MollieSepaRecurringGateway($directDebit, $paymentMethod, $paymentService, $orderInstructionsService, $mollieOrderService, $data, $logger, $notice, $HttpResponseService, $settingsHelper, $mollieObject, $paymentFactory, $pluginId, $apiHelper);
            } elseif ($paymentMethod->getProperty('Subscription')) {
                $gateways[$key] = new MollieSubscriptionGateway($paymentMethod, $paymentService, $orderInstructionsService, $mollieOrderService, $data, $logger, $notice, $HttpResponseService, $settingsHelper, $mollieObject, $paymentFactory, $pluginId, $apiHelper);
            } else {
                $gateways[$key] = new \Mollie\WooCommerce\Gateway\MolliePaymentGateway($paymentMethod, $paymentService, $orderInstructionsService, $mollieOrderService, $data, $logger, $notice, $HttpResponseService, $mollieObject, $paymentFactory, $pluginId);
            }
        }
        return $gateways;
    }
    ...
    protected function instantiatePaymentMethods($container): array
    {
        ...
        foreach ($listAllAvailablePaymentMethods as $paymentMethodAvailable) {
            ...
            $paymentMethods[$paymentMethodId] = $this->buildPaymentMethod($paymentMethodId, $iconFactory, $settingsHelper, $paymentFieldsService, $surchargeService, $paymentMethodAvailable);
        }
        ...
        if (!in_array(Constants::DIRECTDEBIT, array_keys($paymentMethods), \true)) {
            ...
            $paymentMethods[$paymentMethodId] = $this->buildPaymentMethod($paymentMethodId, $iconFactory, $settingsHelper, $paymentFieldsService, $surchargeService, []);
        }
     ...
     */
    public function buildPaymentMethod(string $id, IconFactory $iconFactory, Settings $settingsHelper, PaymentFieldsService $paymentFieldsService, \Mollie\WooCommerce\Gateway\Surcharge $surchargeService, array $apiMethod)
    {
        $transformedId = ucfirst($id);
        $paymentMethodClassName = 'Mollie\WooCommerce\PaymentMethods\\' . $transformedId;
        $paymentMethod = new $paymentMethodClassName($iconFactory, $settingsHelper, $paymentFieldsService, $surchargeService, $apiMethod);
