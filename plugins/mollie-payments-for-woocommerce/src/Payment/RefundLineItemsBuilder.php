...
class RefundLineItemsBuilder
{
    ...
    public function buildLineItems(array $toRefundRemoteItems, array $toRefundItems, $currency, $refundReason)
    {
        ...
        foreach ($toRefundItems as $toRefundItemId => $toRefundItem) {
            ...
            if (in_array($toRefundRemoteItem->status, \Mollie\WooCommerce\Payment\OrderLineStatus::CAN_BE_CANCELED, \true)) {
                $toCancel['lines'][] = $lineItem;
            }
            if (in_array($toRefundRemoteItem->status, \Mollie\WooCommerce\Payment\OrderLineStatus::CAN_BE_REFUNDED, \true)) {
    ...
    private function buildLineItem(WC_Order_Item $toRefundItem, stdClass $toRefundRemoteItem, $currency)
    {
        ...
        if ($toRefundRemoteItemAmount !== $toRefundItemAmount) {
            throw new \Mollie\WooCommerce\Payment\PartialRefundException(esc_html__("Mollie doesn't allow a partial refund of the full amount or quantity of at least one order line. Trying to process this as an amount refund instead.", 'mollie-payments-for-woocommerce'));
        }
