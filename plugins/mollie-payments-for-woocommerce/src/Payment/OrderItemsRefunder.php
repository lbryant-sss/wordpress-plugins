...
class OrderItemsRefunder
{
     ...
     */
    public function __construct(\Mollie\WooCommerce\Payment\RefundLineItemsBuilder $refundLineItemsBuilder, Data $dataHelper, OrderEndpoint $ordersApiClient)
    {
    ...
    private function normalizedRemoteItems(array $remoteItems)
    {
        ...
        foreach ($remoteItems as $remoteItem) {
            $orderItemId = property_exists($remoteItem->metadata, 'order_item_id') && $remoteItem->metadata->order_item_id !== null ? $remoteItem->metadata->order_item_id : 0;
            if (!$orderItemId) {
