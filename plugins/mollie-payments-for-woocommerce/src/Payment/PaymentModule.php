...
class PaymentModule implements ServiceModule, ExecutableModule
{
    ...
    {
        return [\Mollie\WooCommerce\Payment\OrderLines::class => static function (ContainerInterface $container): \Mollie\WooCommerce\Payment\OrderLines {
            $data = $container->get('settings.data_helper');
            $pluginId = $container->get('shared.plugin_id');
            return new \Mollie\WooCommerce\Payment\OrderLines($data, $pluginId);
        }, \Mollie\WooCommerce\Payment\PaymentFactory::class => static function (ContainerInterface $container): \Mollie\WooCommerce\Payment\PaymentFactory {
            ...
            $orderLines = $container->get(\Mollie\WooCommerce\Payment\OrderLines::class);
            return new \Mollie\WooCommerce\Payment\PaymentFactory($data, $apiHelper, $settingsHelper, $pluginId, $logger, $orderLines);
        }, \Mollie\WooCommerce\Payment\MollieObject::class => static function (ContainerInterface $container): \Mollie\WooCommerce\Payment\MollieObject {
            $logger = $container->get(Logger::class);
            assert($logger instanceof Logger);
            $data = $container->get('settings.data_helper');
            assert($data instanceof Data);
            ...
            assert($apiHelper instanceof Api);
            $pluginId = $container->get('shared.plugin_id');
            $paymentFactory = $container->get(\Mollie\WooCommerce\Payment\PaymentFactory::class);
            assert($paymentFactory instanceof \Mollie\WooCommerce\Payment\PaymentFactory);
            $settingsHelper = $container->get('settings.settings_helper');
            assert($settingsHelper instanceof Settings);
            return new \Mollie\WooCommerce\Payment\MollieObject($data, $logger, $paymentFactory, $apiHelper, $settingsHelper, $pluginId);
        }];
    }
    public function run(ContainerInterface $container): bool
    {
        ...
        add_action(\Mollie\WooCommerce\Payment\OrderItemsRefunder::ACTION_AFTER_REFUND_ORDER_ITEMS, [$this, 'addOrderNoteForRefundCreated'], 10, 3);
        add_action(\Mollie\WooCommerce\Payment\OrderItemsRefunder::ACTION_AFTER_CANCELED_ORDER_ITEMS, [$this, 'addOrderNoteForCancelledLineItems'], 10, 2);
        return \true;
    }
