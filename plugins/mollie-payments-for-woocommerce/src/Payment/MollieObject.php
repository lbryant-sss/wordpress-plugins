...
class MollieObject
{
     ...
     */
    protected $pluginId;
    public function __construct($data, Logger $logger, \Mollie\WooCommerce\Payment\PaymentFactory $paymentFactory, Api $apiHelper, Settings $settingsHelper, string $pluginId)
    {
        $this->data = $data;
        $this->logger = $logger;
    ...
    protected function failedSubscriptionProcess($orderId, WC_Payment_Gateway $gateway, WC_Order $order, $newOrderStatus, $paymentMethodTitle, $payment)
    {
        if (function_exists('wcs_order_contains_renewal') && wcs_order_contains_renewal($orderId)) {
            if ($gateway instanceof MolliePaymentGateway) {
                $gateway->paymentService()->updateOrderStatus($order, $newOrderStatus, sprintf(
    ...
    public function getWebhookUrl($order, $gatewayId)
    {
        ...
        $webhookUrl = untrailingslashit($webhookUrl);
        $this->logger->debug(" Order {$orderId} webhookUrl: {$webhookUrl}", [\true]);
    ...
    protected function createBillingAddress($order)
    {
        ...
        $billingAddress->phone = ctype_space($phone) ? null : $this->getFormatedPhoneNumber($phone);
    ...
    public function billingCompanyField($order): ?string
    {
        ...
        return $this->maximalFieldLengths($order->get_billing_company(), self::MAXIMAL_LENGHT_ADDRESS);
    }
    private function checkBillieCompanyField($order)
    {
        ...
        if ($isBillieMethodId) {
            ...
            return $this->maximalFieldLengths($fieldPosted, self::MAXIMAL_LENGHT_ADDRESS);
        }
        return null;
    }
    /**
