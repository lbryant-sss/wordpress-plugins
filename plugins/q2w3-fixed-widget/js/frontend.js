...
var DynamicOffsets = (function (_super) {
    ...
    function DynamicOffsets(el, get_root_offset, need_to_calc_offset, get_prev_sibling, get_next_sibling) {
        ...
        _this.get_height = function () {
            return !_this.el ? 0 :
                Math.round(Math.max(_this.el.clientHeight, _this.el.getBoundingClientRect().height, parseFloat(_this.el.style.height) || 0));
        };
...
var FixedWidget = (function (_super) {
    ...
    FixedWidget.prototype.on_scroll = function (scroll_top) {
        ...
        var need_to_fix = scroll_top > 0 &&
            (scroll_top > this.offsets.root - this.offsets.top);
        if (need_to_fix) {
            var top_1 = Math.min(this.offsets.relative_top - scroll_top, this.offsets.top);
            this.fix(top_1);
            return;
        }
        if (!this.is_pinned) {
