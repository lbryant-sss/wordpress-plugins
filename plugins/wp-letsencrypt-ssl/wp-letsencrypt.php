...
if ( !defined( 'WPLE_PLUGIN_VER' ) ) {
    define( 'WPLE_PLUGIN_VER', '7.8.5.3' );
}
...
if ( function_exists( 'wple_fs' ) && !function_exists( 'wple_fs_custom_connect_message' ) ) {
    function wple_fs_custom_connect_message(  $message  ) {
        ...
        return 'Howdy ' . ucfirst( $current_user->user_nicename ) . ', <br>' . sprintf(
            __( 'Due to security nature of this plugin, We <b>HIGHLY</b> recommend you opt-in to our security & feature updates notifications, and %snon-sensitive diagnostic tracking%s to get BEST support. If you skip this, that\'s okay! %sWP Encryption%s will still work just fine.', 'wp-letsencrypt-ssl' ),
            '<a href="https://freemius.com/wordpress/usage-tracking/5090/wp-letsencrypt-ssl/" target="_blank">',
            '</a>',
            '<b>',
            '</b>'
        );
