...
<p>
For this example we are composing a gallery filtered by just one data selection parameter, <code>attachment_category=abc</code>. You can use as many data selection parameters as you need for your application, but if you use <code>allow_empty_gallery=false</code> in the <code>[mla_download_checklist]</code> shortcode make sure to add exactly the same parameters to both shortcodes in the form. The <code>mla_markup=gallery-checklist</code> parameter links the shortcode to the custom markup template created for this application, as described above.
</p>
