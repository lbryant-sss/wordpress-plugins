...
class BVProtectIpstore_V588 {
	...
	function __construct($storage_type = BVProtectIpstore_V588::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V588::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V588();
		} else {
			$this->storage = new BVProtectIpstoreDB_V588();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V588::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V588::STORAGE_TYPE_DB) {
