...
class BVProtectIpstore_V591 {
	...
	function __construct($storage_type = BVProtectIpstore_V591::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == BVProtectIpstore_V591::STORAGE_TYPE_FS) {
			$this->storage = new BVProtectIpstoreFS_V591();
		} else {
			$this->storage = new BVProtectIpstoreDB_V591();
		}
	}

	public static function uninstall() {
		BVProtectIpstoreDB_V591::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == BVProtectIpstore_V591::STORAGE_TYPE_DB) {
