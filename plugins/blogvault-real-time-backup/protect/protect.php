...
if (!class_exists('BVProtect_V588')) :
...
class BVProtect_V588 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V588::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V588::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V588::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V588($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V588::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V588::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V588::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V588::uninstall();
		BVProtectFW_V588::uninstall();
		BVProtectLP_V588::uninstall();

		BVProtect_V588::removeWPPrepend();
		BVProtect_V588::removePHPPrepend();
		BVProtect_V588::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V588::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V588::removeHtaccessPrepend();
		BVProtect_V588::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V588::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V588::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V588::rrmdir($mc_data_dir);
	}
}
endif;